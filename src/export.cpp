...
Rcpp::List dgtf_default_algo_settings(const std::string &method)
{
    ...
    switch (AVAIL::to_algo(method_name))
    {
    ...
    default:
        throw std::invalid_argument(
            "dgtf_default_algo_settings: no default settings for method " +
            method_name);
        break;
    }
    ...
    const bool &add_y0 = false)
{
    ...
    const AVAIL::Algo algo = AVAIL::to_algo(algo_name);
    ...
    {
        throw std::invalid_argument(
            "dgtf_infer: method " + algo_name + " has no inference engine.");
        break;
    }
    } // switch by algorithm
