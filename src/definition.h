...
public:
    ...
    /**
     * @brief Look up an inference algorithm by name.
     *
     * Indexing `algo_list` with `operator[]` inserts a value-initialized
     * entry when the key is absent, which resolves to the first enumerator
     * rather than reporting an error. This helper names the unknown method
     * instead.
     *
     * @param name Algorithm name, already lowercased by the caller.
     * @return The matching Algo enumerator.
     */
    static Algo to_algo(const std::string &name)
    {
        auto it = algo_list.find(name);
        if (it == algo_list.end())
        {
            throw std::invalid_argument("Unknown inference method: " + name);
        }
        return it->second;
    }
...
private:
    static std::map<std::string, Algo> map_algorithm()
    {
        ...
        ALGO_MAP["vb"] = Algo::HybridVariation;
