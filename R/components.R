...
#' `obs_nbinom_p()` (number of successes and probability form).
...
#'
#' The discretized probability mass function that weights past
#' observations in a sliding window transfer function.
#'
#' \describe{
#'   \item{\code{lag_lognormal(meanlog, sigma2, sdlog, residual_prob)}}{Discretized
...
#'   \item{\code{lag_nbinom(r, kappa, residual_prob)}}{Discretized
...
#'   the discretized lag distribution is allowed to drop when truncating.
...
#'   \eqn{p}. It is deprecated in \code{lag_lognormal()} and
#'   \code{lag_nbinom()}, where it is accepted but ignored, since those
#'   constructors derive the truncation length from
#'   \code{residual_prob}.
#'
...
#' - `sys_shift()`: shift matrix for windowed Hawkes and distributed lag
#'   models.
#' - `sys_nbinom()`: nonlinear evolution for the Solow distributed lag
#'   model, where `kappa` is the geometric or negative binomial decay
#'   parameter.
#'
...
as_component <- function(x, kind) {
    ...
    stop(sprintf("`%s` must be a dgtf_%s object or a recognized string.",
