...
# `dgtf_fit` objects in `dgtf_compare_plot()`. EpiEstim is listed under
# Suggests rather than Imports, so this file guards the call with
# `requireNamespace()` and errors with an install hint when the package
# is missing.
...
#' in a format \code{dgtf_compare_plot()} consumes. The return value is a
#' `length(y) x 3` numeric matrix with columns `lower`, `median`, and
#' `upper`, padded with leading NAs so that row `i` corresponds to time
#' `i` in the input series. This follows the same convention as
#' `dgtf_fit$fit$psi`.
#'
#' The wrapper exists so that a renewal equation baseline can be placed
#' next to `dgtf_fit` objects in the comparison plot.
#'
#' ```
...
#'   typically want to deseasonalize `y` upstream before calling
#'   EpiEstim, since the renewal model assumes a smooth incidence
#'   signal.
#' @param mean_si,sd_si Serial interval **mean** and **standard
#'   deviation** in the same time units as `y`. These are the mean and
#'   standard deviation, not the variance.
#' @param method One of three options.
#'   * `"parametric_si"` (default), which takes the serial interval mean
#'     and standard deviation as known.
#'   * `"uncertain_si"`, which propagates uncertainty in the serial
#'     interval mean and standard deviation, using `mean_sd_si` and
#'     `sd_sd_si` as prior standard deviations.
#'   * `"wallinga_teunis"`, the case reproduction number method of
#'     Wallinga--Teunis.
...
#'   natively reports the 0.025 and 0.975 quantiles. For any other level
#'   the bounds are derived from a normal approximation to the posterior,
#'   `mean` plus or minus `z * Std(R)`, and a warning is emitted.
#' @param align Alignment of the EpiEstim window summary in the returned
#'   matrix. Use `"center"` to place each estimate near the middle of its
#'   window, or `"right"` to place it at the window end.
...
#'   `length(y) x 3` numeric matrix with columns `lower`, `median`, and
#'   `upper`. The first `window` rows are `NA`, because no estimate is
#'   available yet. The method, the level, and the serial interval
...
epiestim_baseline <- function(y,
    ...
    # parametric and uncertain branches and Mean(R) for Wallinga--Teunis,
    ...
    bands <- if (col_lo %in% names(out) && col_hi %in% names(out)) {
        ...
        # Non-default level path. EpiEstim reports only the 0.025 and 0.975
    ...
    if (align == "right") {
        ...
        # Center alignment shifts left by floor((window - 1) / 2) relative
        # to the right edge.
