...
#' observed counts and, when the PPC carries a reference \eqn{R_t}
#' trajectory, the corresponding \eqn{R_t} recovery metrics.
#'
...
#' @param ... For fit lists, the optional arguments \code{level},
#'   \code{nrep}, \code{Rt_truth}, \code{truth}, and
#'   \code{force_recompute} control the posterior predictive computation
#'   performed on the fly and the optional comparisons against truth.
#'   Extra arguments are ignored for PPC and forecast lists.
#'
...
#'
#' For a compact summary across fits, use [`dgtf_compare()`], supplying
#' `truth` when the recovery columns are wanted.
#'
...
# Internal helper producing the summary rows for a single fit. It
# dispatches to `param_recovery()` when truth is provided, and otherwise
# computes the posterior summaries directly from
# `.dgtf_static_draws()`.
