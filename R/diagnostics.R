...
#' Plot the ELBO trajectory of a VB fit
#'
#' @param object A `dgtf_fit` produced with `method = "vb"`.
#' @export
plot_elbo <- function(object) {
    if (!inherits(object, "dgtf_fit") || object$method != "vb")
        stop("`object` must be a vb `dgtf_fit`.", call. = FALSE)
    elbo <- object$fit$elbo %||% object$fit$elbo_trace
    if (is.null(elbo))
        stop("ELBO trace not stored in this fit.", call. = FALSE)
