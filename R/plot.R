...
# Soft replacement for match.arg(). Time series targets form an enum,
# diagnostic targets such as "marglik" have their own dispatch path, and
# any other single string is treated as a static parameter name, which is
...
# Lazy PPC fallback. Triggered only when Rt or yhat is requested, no PPC
# is attached, and, in the Rt case, psi draws are unavailable.
...
.dgtf_band <- function(x, what, level = 0.95, time = NULL, name = NULL) {
    ...
    # Branch 2b: dgtf_forecast, which supports yhat only
...
#        matching .truth_pattern, since truth is rendered separately).
...
#   name   character, method or source label (recycled)
#   index  integer, 1..p_dim, identifying the entry of a vector parameter
#   value  numeric, a single draw or the truth value
#   kind   character, either "draws" or "truth"
#
# Dispatches on class(x):
...
# outliers do not drown the convergence signal.
#
# Layers (bottom -> top):
...
# iterations do not render the convergence line invisible. Outliers
...
#'
#' Time series plot of the forecast quantile band, drawn as a median
#' line with a ribbon, with an optional truth overlay as a dashed black
#' line. This follows the convention of the other dgtf time series
#' plotting methods.
#'
#' Single-step forecasts, obtained by setting `only_last = TRUE` in the
#' original [`dgtf_forecast_fit()`] call, are rendered as a point with
#' an interval rather than as a line with a ribbon, since a single pair
#' of horizon and value is all there is to show. The truth, when
#' present, is drawn as a black cross at the same time step.
#'
...
#'   [`dgtf_forecast_fit()`], namely at least `horizon` entries for the
#'   full trajectory and at least one for `only_last`). Defaults to
...
#' @param xlab,ylab,main Axis and title labels. `NULL` falls back to
#'   `"Forecast step"` and `expression(hat(y))`.
...
plot.dgtf_forecast <- function(x,
    ...
    # to draw a meaningful line or ribbon.
    if (only_last || h == 1L) {
        df <- data.frame(
...
#' In-sample fit and out-of-sample forecast on one plot
#'
#' Plots the posterior predictive band for `y` over the in-sample
#' window and, optionally, the forecast band for `y` over the
#' out-of-sample window taken from a `dgtf_forecast`, with the two
#' distinguished by color. Truth values for either segment are overlaid
#' as a single dashed black line spanning both windows.
#'
#' The in-sample band can come from either of two sources.
#'   * A `dgtf_fit`, whose `attr(., "ppc")` is reused when present and
#'     otherwise computed by [`posterior_predict()`] on the fly. The
#'     observed `y` is used as the in-sample truth by default.
#'   * A `dgtf_ppc`, which is used directly. There is no automatic
#'     in-sample truth in this case, because the PPC object does not
#'     carry `y`. Supply one through `truth_in` when it is wanted.
#'
...
#'   regardless of `truth_in` and `truth_out`. Default `TRUE`.
#' @param time_in,time_out Optional numeric vectors giving the x-axis
#'   values for the two segments, such as real dates coerced to
#'   numeric. They default to `0:(n-1)` in sample and `n:(n+h-1)` out of
#'   sample, with `n = length(fit$y)` and `h = forecast$horizon`, so
#'   that the two segments connect.
...
#'   on-the-fly PPC.
#' @param in_color,out_color Hex colors for the two bands. Default
#'   to Okabe--Ito blue and vermillion, so that the two are easy to tell
#'   apart.
...
plot_dgtf_y <- function(x,
    ...
    #    the default increment so that dates and integer indices both work.
    band_out <- NULL
    t_out <- numeric(0)
    h_out <- 0L
    if (!is.null(forecast)) {
...
#' of model variants. For each method the function draws an in-sample
#' posterior predictive band, taken from the fit's PPC either cached or
#' computed on the fly. When a forecast is supplied, it also draws a
#' forecast band continuing the time axis past the train/test
#' boundary.
#'
#' Each method gets a single color applied to *both* its in-sample
...
#' second color channel. This keeps the legend readable across many
#' methods.
#'
...
#'   `NULL` (default) auto-fills with the `$y` of the first fit (assumed
...
#' @param time_in,time_out Optional numeric vectors giving the x-axis
#'   values for the two segments. They default to `0:(n-1)` in sample
#'   and `n:(n+h-1)` out of sample, so that the segments connect.
#' @param level,nrep Forwarded to [`posterior_predict()`] for fits
#'   without a cached PPC.
#' @param palette Optional named character vector of colors.
#' @param alpha Ribbon transparency (default 0.4).
