...
#' Hybrid variational approximation control
#'
#' Tuning parameters for the hybrid variational algorithm, selected in
#' [`dgtf()`] with `method = "hva"` or its synonym `method = "vb"`.
#'
...
#'   C++ engine. Use these for options that the named arguments above do
#'   not cover, such as `use_discount` and `discount_factor`.
#'
...
#' MCMC control
#'
#' Tuning parameters for the disturbance sampler, selected in [`dgtf()`]
#' with `method = "mcmc"`.
#'
#' @param iter Number of retained post-warmup MCMC iterations.
#' @param warmup Number of burn-in iterations, which are discarded.
#' @param thin Thinning interval.
#' @param ... Forwarded to the C++ engine. The HMC tuning options
#'   `hmc_step_size_init`, `hmc_target_accept`, and
#'   `hmc_dual_averaging` are passed this way.
#'
...
#'
#' Tuning parameters for the sequential Monte Carlo smoothers. These
#' engines are reached through the low-level [`dgtf_infer()`] interface
#' with `method` set to `"smc"`, `"ffbs"`, `"tfs"`, `"mcs"`, or `"pl"`,
#' rather than through [`dgtf()`].
#'
#' @param n_particle Number of particles.
#' @param n_backward Number of backward simulations, used by the
#'   smoothing variants.
#' @param resample Resampling strategy, one of `"systematic"` (default),
#'   `"multinomial"`, or `"stratified"`.
#' @param ess_threshold Effective sample size threshold that triggers
#'   resampling.
...
#'
#' Tuning parameters for the linear Bayes approximation, reached through
#' the low-level [`dgtf_infer()`] interface with
#' `method = "linear_bayes"`.
#'
