...
#'   coverage and interval score calculations in
...
#' @examples
#' \dontrun{
#' # An external simulation is any code that draws from the generative
#' # process described by `mod`. The call below shows the expected shape.
#' mod <- dgtf_hawkes()
...
as_dgtf_sim <- function(x, model, ...) {
    ...
    message(
        ...
        "values from a faithful forward simulation under `model`, since ",
