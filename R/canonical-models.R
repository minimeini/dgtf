...
# the default with sys = "shift", which gives the truncated sliding
# window form controlled by residual_prob on the lag component. The
# truncated form is faster per step but introduces a small approximation
# in the lag tail. The default iterative form represents the lag
# distribution exactly and matches the nL = nT convention used by MCMC.
...
#'
#' @param r Integer. Number of successes in the negative binomial lag
#'   distribution. Default 4. Under \code{sys = "nbinom"} this argument
#'   is structural, because it sets the state dimension \eqn{r+1}. It
#'   must therefore be held fixed at construction, and a prior on
#'   \code{par2 = r} is refused by HVA.
#' @param kappa Numeric. Failure probability in (0, 1). Default 0.5.
#'   Can be inferred under either system form.
#' @param sys Either \code{"nbinom"} (default), which gives the exact
#'   iterative form, or \code{"shift"}, which gives the truncated
#'   sliding window form. The iterative form is recommended for fitting
#'   and matches the internal representation used by MCMC. The truncated
#'   form applies the same residual probability truncation as
