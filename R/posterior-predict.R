...
#'   total number of posterior predictive draws is `nrep` times the
#'   number of posterior samples, where the latter is the second
#'   dimension of `object$fit$psi_stored` or `object$fit$Theta`.
#' @param Rt Optional vector of reference intensity values, of the same
#'   length as `y`, such as `softplus(sim$psi)` in a simulation study.
#'   Pass `NULL` (default) to skip the reference comparison.
#' @param ... Unused.
#'
#' @param level Credible interval level used when reporting coverage,
#'   interval width, and the interval score (default `0.95`).
#'
#' @return An object of class `dgtf_ppc` (a list). Four elements are
#'   always present.
#'   * `chi`, the averaged chi-square discrepancy of `y`.
#'   * `Rt`, an `(ntime + 1) x 3` matrix of `R_t` posterior quantiles at
#'     `(alpha/2, 0.5, 1 - alpha/2)` with `alpha = 1 - level`.
#'   * `width_Rt`, the mean width of the `R_t` credible bands.
#'   * `level`, the credible interval level used.
#'
#'   Three further elements are added when `nrep > 0`.
#'   * `crps`, the averaged continuous ranked probability score for `y`.
#'   * `yhat`, an `(ntime + 1) x 3` quantile matrix of the posterior
#'     predictive distribution of `y`.
#'   * `coverage_yhat`, `width_yhat`, and `interval_score_yhat`, the
#'     Winkler interval score for `y` together with its components.
#'
#'   When a reference `Rt` trajectory is supplied, as in a simulation
#'   study, the recovery metrics `mae_Rt`, `rmse_Rt`, `coverage_Rt`, and
#'   `interval_score_Rt` are added as well.
#'
#' @export
#' @examples
#' \dontrun{
...
#' @param nrep Number of replicate draws per posterior sample. The
#'   total number of forecast draws is `nrep` times the number of
#'   posterior samples.
#' @param ypred_true Optional numeric vector of held-out future
#'   observations, of length at least `h`. When supplied, the scoring
#'   metrics `coverage`, `chi`, and `crps` are computed against the last
#'   `h` elements, or against the last single value when
#'   `only_last = TRUE`.
...
#' @param ... Unused.
#'
#' @return An object of class `dgtf_forecast` (a list) with the
#'   following elements.
#'   * `quantiles`, a numeric matrix with columns `lower`, `median`, and
#'     `upper` at the 95% level. It has `h` rows by default, or one row
#'     when `only_last = TRUE`.
#'   * `level`, the credible interval level, currently fixed at `0.95`.
#'   * `horizon`, the requested forecast horizon `h`.
#'   * `only_last`, the flag described above, and `nsample`, the total
#'     number of forecast draws.
#'   * `samples`, a matrix of posterior predictive draws, present only
#'     when `return_samples = TRUE`.
#'   * `ypred_true`, the supplied vector of held-out observations, when
#'     one was given.
#'   * `coverage`, `chi`, and `crps`, the scoring metrics, present only
#'     when `ypred_true` is supplied.
#'
#' @export
#' @examples
#' \dontrun{
