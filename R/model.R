#' Build a DGTF model
#'
#' Assembles the components of a dynamic generalized transfer function
#' (DGTF) model. The returned object carries all structural choices and
#' all fixed parameter values, and is the object on which `dgtf()` and
#' `dgtf_simulate_model()` operate.
#'
#' Components may be supplied either as full constructor objects
#' (`obs_nbinom()`, `lag_lognormal(...)`, and so on) or as the
#' corresponding string shortcuts (`"nbinom"`, `"lognorm"`). The
#' constructor form is the more general of the two, since it also
#' carries the component parameter values.
#'
#' Anything supplied as a numeric value, such as `intercept = 0` or
...
#'   since the intercept is the single seasonal effect under period 1.
...
#'   `seasonality` is `NULL` or `seas_none()`, since otherwise the
#'   seasonal effects already carry the level.
...
#'   parameters. The default `c(0, 30)` sets a zero location and an
#'   overdispersion of 30 for the negative binomial observation model.
...
#'
#' # Discretized Hawkes with weekly seasonality
#' dgtf_model(
...
as_settings.dgtf_model <- function(model) {
    ...
    # Residual probability threshold used when discretizing the lognormal
    # and negative binomial lags. The C++ engine takes the complementary
    # `lag_prob_thres`, so the truncation length L is the smallest integer
    # with CDF >= lag_prob_thres.
    param_list <- list(
        obs = model$obs_init,
        lag = lag_params,
        err = list(
